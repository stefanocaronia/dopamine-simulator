...
- **Methylphenidate** (30 s): DAT1 block. 85% of reuptake attempts fail, transporters move at 0.3× speed. A failed attempt is decided before the transporter reaches out (`d.state === 'blocked'`, 0.6–1.4 s barred and idle, brighter drug-coloured halo), so under the drug the DAT1 sit barred instead of reaching out and missing; the membrane-zone reuptake probability is scaled by the same block. Simulated side effects: sleep debt grows ×1.5 while active; less reuptake means less recycling (emergent); after the dose a 15 s rebound with DAT1 speed ×1.3.
