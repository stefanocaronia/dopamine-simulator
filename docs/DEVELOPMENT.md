...
- The two percentages in that toast (`activeShown`, `passShown`) come from slow averages (about 7 s) sampled once a second, quantized to 5% and held with hysteresis (`stickyPct`): the shown value only moves when the true value is more than one and a half steps away, so the number does not dance. `0` and `100` are reserved for the real extremes (never produced by rounding, which is capped at 5–95), because the toast turns a zero into words: `u.receptive.none` when the neurons are never receptive, `u.receptive.nopass` when they are receptive but no cortical impulse fired them.
