...
- The COMT enzymes move like Pac-Man: only along the axes, at a constant `COMT_SPEED`, turning at a junction (`comtTurn`) when the turn timer runs out or a wall is `COMT_LOOK` ahead. At a junction they score the four directions: toward the nearest free molecule within `COMT_SEE`, away from the other enzymes, and against doubling back. It is not just decoration — straight-line patrolling covers more ground than the old damped random walk, whose jitter partly cancelled itself, so captures went up by about half in the neurotypical brain at high stimulus and stayed put in ADHD. The per-enzyme rate is unchanged, so the declared ×2.4 ADHD activity still holds.
- The active zone (SNAP25) is the flat face of the terminal membrane: `termFace(t)` returns its ends, which breathe with `termWob` exactly like the outline, and the green stimulus/fusion glow is stroked along it. It used to be a fixed `fillRect` next to the border, which drew a second parallel line and stayed put while the membrane moved.
- Rendering: glow through cached sprites (no per-particle `shadowBlur`); trails as spark dust: every moving molecule stamps a few small, sparse, jittered glow dots along the stretch it covered in the frame onto a persistence canvas faded each frame with `destination-out` (slow diffusing molecules leave almost nothing, molecules in flight a short tail of sparks; no line segments between frames). The terminal lights up cyan at every spike that arrives (`preGlow`/`preFlash` envelope: hold 0.04 s, falloff τ 0.10 s; a tonic spike lights the membrane at half intensity, a burst spike fully, so bursts stand out and a 20 Hz burst keeps the membrane lit) and the active zone flashes green at every fusion (`snapGlow`); a hover highlight; every label goes through `T()`. Receiving cells light up as an impulse: `n.glow` is the light envelope (full for 0.15 s after the crossing, then 0.55 while the neuron stays receptive, exponential falloff with τ 0.35 s once it drops below threshold) and `n.flash` is the yellow flash of the gauge arc at the crossing; the label follows the light, not the raw state.
- Molecules have four ends: taken back by DAT1 (then recycled by VMAT2 or destroyed by MAO), destroyed by COMT in the cleft, or **dispersed** — 2 s free in the cleft and they fade out green (`state 'expire'`, `stats.lost`, `rateLostCount`). Dispersal is not destruction: it feeds its own counter and its own rate (`displayLostRate`), never the red ✕ one, and the molecule keeps the ordinary green tooltip. At 100% stimulus in the neurotypical brain dispersal is about a third of all clearance, so folding it into "destroyed" made that counter mean nothing.
- The stimulus toast carries only what a reader needs: release rate, the two percentages, and the reserve trend. Spikes/s, bursts/s and firings/s live in the axon and cortical-input tooltips, where someone who wants the mechanism goes looking.
- The two percentages in that toast (`activeShown`, `passShown`) are built in three stages, every half second: a **sliding window** of `PCT_WINDOW` = 3 s (`activeBox`, `passBox`, exact windowed means, not exponential — the window empties cleanly, so a zero is a real zero after 3 s of silence instead of the tail of an average that needs half a minute to get there); an **adaptive smoothing** of that window (`PCT_FAST` 0.5 when it moves by more than `PCT_JUMP` = 0.15, i.e. the user moved a slider, `PCT_SLOW` 0.08 otherwise, i.e. simulation noise); and **hysteresis** on the displayed value (`stickyPct`, 5% steps, moves only when the true value is more than one and a half steps away). `0` and `100` come straight from the window, bypassing the smoothing, and rounding can never produce them (it is capped at 5–95), because the toast turns a zero into words: `u.receptive.none` when the neurons are never receptive, `u.receptive.nopass` when they are receptive but no cortical impulse fired them. Measured: the number starts moving 2–3 s after a slider change and settles by 5 s, while at a steady stimulus it changes only a handful of times a minute.
- In-canvas rates, the reserve trend (`vesTrend`) and the depletion and tolerance-recovery estimates in the toasts are all per **simulated** second: counts are taken over a 0.5 s real window and divided by the simulated time elapsed in it (`rateSim`), so the numbers match the texts (3 Hz tonic, up to 30 vesicles/s) at any time scale, and smoothed with a moving average (factor 0.3 per window, about 1.5 s) because packetized release made a bare 0.5 s window swing between 2/s and 25/s.
- Receiving neurons: dopamine only makes them **receptive** (signal above threshold: yellow gauge, `n.active`, the averaged "% of time receptive"). What fires them is a **cortical input**: each cell has a fibre entering from the right corridor (`POST.ctxW`) with glutamatergic pulses at `CTX_BASE_HZ` + `CTX_STIM_HZ` × stimulus (1–4 Hz per neuron, `n.inPulses`, travel `CTX_TRAVEL` 0.3 s); on arrival the neuron fires if receptive (cyan impulse light `n.glow`, hold 0.06 s, τ 0.15 s, cyan ring, `stats.fires`, `displayFireRate` in the cortical-input tooltip) or the pulse dies on the membrane (grey ring, `stats.misses`). Same input, different outcome: that is the point. Colour code: yellow = threshold/receptive (`C.recept`, CSS `.c-act`), cyan = a neuron firing (`C.active`, `.c-fire`), on both sides of the cleft. The gate is drawn with the sign of the direct pathway (D1-like facilitation) while the receptors counted are D2; declared in Model vs reality.
- The donut gauge scale is `max(ACT_THRESHOLD × 2.5, threshold × 1.15)`, not a fixed 2.0: sleep debt and hangovers push `thresholdNow()` up to 3.12, and with a fixed scale the tick saturated at 98% while the arc could pass it with the neuron still silent.
...
- **Methylphenidate** (30 s): DAT1 block. 85% of reuptake attempts fail, transporters move at 0.3× speed. A failed attempt is decided before the transporter reaches out (`d.state === 'blocked'`, 0.6–1.4 s barred and idle, brighter drug-coloured halo); the diagonal bar is drawn only on transporters actually in that state, not on every transporter while the drug is present, so under the drug the DAT1 sit barred instead of reaching out and missing; the membrane-zone reuptake probability is scaled by the same block. Simulated side effects: sleep debt grows ×1.5 while active; less reuptake means less recycling (emergent); after the dose a 15 s rebound with DAT1 speed ×1.3.
- **Exercise** (12 s): +1.5 bursts/s (about +15 molecules/s, reduced by sleep debt), every vesicle costs 0.02 while it lasts, synthesis +0.5/s (reduced by sleep debt).
- **Re-dosing**: clicking caffeine, medication, exercise or a substance while its effect is active restarts the full duration (and cancels the pending after-effect for substances), so chain use is possible and the D2 loss of substances keeps running. The easy-rewards button is a switch with a lever inside; its toast counts how long it has been on (`scrollOnAt`).
- **Easy rewards** (toggle, formerly "scrolling"): one burst per simulated second of 4–6 spikes carrying 12 vesicles in total at 0.03 cost each (cue-triggered, low effort). While on, D2 sensitivity `d2Sens` drops by 0.02/s down to 0.3; off, it recovers by 0.004/s. The effective receptor count per neuron is `round(d2Count × d2Sens)` (min 2) and `rebuildReceptors()` runs whenever it changes, so receptors visibly disappear and return. A "D2 tolerance" toast stays while `d2Sens < 0.92`.
...
- **Firing** (`emitSpike`, `queueBurst`, `fireSpike`): the tonic pacemaker emits single spikes and stays silent while a burst is queued; a burst is 4–6 spike descriptors pushed onto `spikeQ` and emitted at 20 Hz (30 Hz when more than 8 are waiting, i.e. overlapping bursts). Every spike is an `apPulses` entry that carries its vesicle count and cost, travels in simulated time and fuses on arrival at `TERM.xJ + 0.18·PRE.w`, at one random fusion site of the active zone (the molecules of one spike spawn within ±7 px of it, so a burst reads as a few puffs). The tonic pacemaker gives caffeine and methylphenidate something to act on at low stimulus.
...
- **Kids mode** (`kidMode`, toggle 🧒 in the header, `?kid=1` in the URL so the link is shareable, remembered in `localStorage` `dopa.kid`): `T()` looks up `<key>@kid` first and falls back to the normal key, so only the strings that mention gambling, porn or drugs have a kid variant (14 keys); `body.kid` hides the four substance buttons; `renderDocs()` skips the sections in `KID_HIDE` (the substances section of How it works); switching it on sets the age to `KID_AGE` = 12 and clears active substances, switching it off returns to 30. The model itself does not change.
...
- **Substances** (`SUBST` in model.js, one dose per click, real-time timers; while the effect lasts `d2Sens` drops by `des` per real second):
  ...
  The extra release (`boost`, molecules/s) is independent of the stimulus and is delivered as extra bursts (boost ÷ 10 bursts/s), so the substances act even at rest; easy rewards stay at one cheap 12-vesicle burst per second. Desensitization (`des`, in real seconds) runs for as long as the effect lasts, so one dose costs 16% (cannabis) to 50% (cocaine) of the receptors, and the bar visibly drops while the toast is up.
...
| Stimulus | Novelty, challenge, fear: burst rate (stimulus × 3 bursts/s of 4–6 spikes, 2 vesicles each), up to 30 vesicles/s on top of the 3 Hz tonic pacemaker |
...
| 🏃 Exercise | 12 s: +1.5 bursts/s (about +15/s) at 0.02 cost per vesicle, synthesis +0.5/s (less with sleep debt) |
| 💤 Sleep | Reset of debt, reserve, cleft and timers; D2 sensitivity recovers only +10% |
| 📱 Easy rewards | Toggle: one cheap 12-vesicle burst per simulated second; D2 sensitivity −0.02/s down to 30%, recovery +0.004/s once off |
| 🚬 Nicotine | 20 s: +1 burst/s (about +10/s) and ×1.30, D2 −1%/s; then a 15 s dip (×0.85); sleep debt ×1.2 |
| 🌿 Cannabis | 40 s: +0.6 bursts/s (about +6/s) and ×1.15, D2 −0.4%/s; then synthesis ×0.7 for 60 s |
| 🍷 Alcohol | 30 s: +0.9 bursts/s (about +9/s) and ×1.25, D2 −0.8%/s; then threshold ×1.2 for 30 s; sleep debt ×1.3 during and after |
| ❄️ Cocaine | 20 s: DAT1 block 95%, speed ×0.3, +1.4 bursts/s (about +14/s), D2 −2.5%/s; then DAT1 ×1.5 and threshold ×1.3 for 20 s; sleep debt ×2 |
...
- Sanity numbers (seed 20260908 of the tests, speed 1, 30 s after a 10 s warm-up): fraction of time with receptive neurons 0.28 / 0.76 / 1.0 for the neurotypical brain at 10 / 30 / 100% stimulus, 0.07 / 0.25 for ADHD at 10 / 30%; firings 0.8 / 3.6 / 12 per second vs 0.1 / 1.5 in ADHD; methylphenidate lifts ADHD 10% to about 0.27, caffeine lifts neurotypical 10% to about 0.5; a dose of cocaine costs about 50% of D2 sensitivity, 20 s of easy rewards 40%; COMT eats about 15 (neurotypical) or 17 (ADHD) molecules in 30 s at 60%, averaged over three seeds — a rare event, so a single seed swings widely (9 to 27 here).
...
- [ ] Biology, see `docs/fedelta-biologica.md`: presynaptic D2 autoreceptors (release inhibition when cleft dopamine is high); dopamine pauses on disappointing rewards and variable burst size (prediction error); the two striatal pathways drawn separately (D1 "go" cells facilitated, D2 "brake" cells quieted); region selector (striatum vs prefrontal cortex) changing DAT, COMT and NET weights; "raw materials" (diet, iron) as a synthesis multiplier and chronic stress (short-term release boost, long-term D2 loss) as new environmental factors
...
- v3.7, 2026-09-08: receiving neurons fire only when a cortical input pulse arrives while they are receptive; cortical fibres in a right-hand corridor with pulses at 1 Hz + 3 Hz × stimulus; firings/s reported to the reader; colour code split: yellow = receptive (gauge, label, texts), cyan = firing (cell light) on both sides; "go and brake" (direct/indirect pathway) explained in How it works and the D2/D1 sign simplification declared in Model vs reality; the receptive line of the stimulus toast also says which share of the cortical impulses gets through (`passPct()`); headless integration tests in `tests/` with a CI workflow; then an audit pass: dispersed molecules split from destroyed ones everywhere (counter, rate, tooltip, molecule tooltip), adaptive gauge scale, bar only on transporters actually blocked, DAT1 tooltip showing the live speed and the block for cocaine too, D2 counts in the texts following the age, reserve trend per simulated second, depletion text at 20% and depletion estimate aimed at the knee instead of an empty reserve that never happens

- v3.7.1, 2026-09-09: audit fixes (dispersed molecules split from destroyed ones, adaptive gauge scale, bar only on transporters actually blocked, live DAT1 tooltip, age-aware D2 counts in the texts, reserve trend per simulated second); the stimulus toast lost the spikes/bursts/firings line and says a zero in words; the two percentages got a sliding window plus adaptive smoothing and hysteresis, steady at a fixed stimulus and moving within 2-3 s of a slider change; the SNAP25 active zone is the membrane face itself, drawn along the wobble, instead of a fixed blue strip beside it; the COMT enzymes move like Pac-Man, only along the axes
