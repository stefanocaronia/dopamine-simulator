...
- Rendering: glow through cached sprites (no per-particle `shadowBlur`); trails through a persistence canvas faded each frame with `destination-out` (long jumps leave a dot, not a line); a hover highlight; every label goes through `T()`. Receiving cells light up as an impulse: `n.glow` is the light envelope (full for 0.15 s after the crossing, then 0.55 while the neuron stays receptive, exponential falloff with τ 0.35 s once it drops below threshold) and `n.flash` is the yellow flash of the gauge arc at the crossing; the label follows the light, not the raw state.
