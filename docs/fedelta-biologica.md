...
| Caffeina | Antagonista A2A: soglia dei neuroni riceventi −20%, rilascio +10%, dimezza la penalità del debito di sonno, per 25 s. Non tocca i DAT. | È un **antagonista dei recettori dell'adenosina A2A**: negli eteromeri A2A–D2 dello striato aumenta l'affinità e il segnale dei D2 e alza modestamente il rilascio (Ferré 2008). L'adenosina che si accumula con la veglia è proprio ciò che la caffeina blocca: da qui l'effetto "maschera la stanchezza". Le grandezze (−20%, +10%) sono illustrative. | ●●●○ |
| Metilfenidato | Blocca i DAT per 30 s: l'85% delle ricaptazioni fallisce, i trasportatori vanno a 0,3× della velocità. | Il metilfenidato è un inibitore del DAT (e del NET): aumenta la dopamina extracellulare nello striato e nella corteccia prefrontale. È il farmaco di prima scelta per l'ADHD. Le anfetamine, non simulate, in più invertono il DAT e forzano il rilascio. | ●●●○ |
...
- **Attività ad alto rilascio e bassa fatica (scrolling, social, gioco compulsivo)**: nella realtà contano tre cose che il modello non ha: (1) le raffiche fasiche scatenate dai **segnali anticipatori** (notifica, pull-to-refresh) più che dalla ricompensa; (2) la **ricompensa variabile** (slot machine), che mantiene alto l'errore di predizione; (3) la **regolazione verso il basso dei D2** con stimolazione ripetuta (tolleranza), che alza la soglia per tutto il resto e si recupera in settimane. La "dipendenza da dopamina" e la "disintossicazione dalla dopamina" sono espressioni sbagliate: la dopamina non si accumula né si smaltisce; ciò che cambia è la sensibilità dei recettori e l'apprendimento dei segnali. Una possibile implementazione: un pulsante "Scrolling" che produce raffiche frequenti a basso costo, un parametro lento `d2Sensitivity` che scende con l'esposizione e risale con l'astinenza, e un toast che spiega la tolleranza.
...
2. **Rilascio tonico** minimo anche a stimolo zero; lo stimolo diventa la componente fasica.
3. **Selettore di regione** (striato / corteccia prefrontale) che cambia i pesi di DAT, COMT e NET.
4. **Tolleranza dei D2** per le attività compulsive (vedi sopra).
